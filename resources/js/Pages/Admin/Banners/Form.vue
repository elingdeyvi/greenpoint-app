...
const record = computed(() => props.banner ?? props.item);
const isEdit = computed(() => !!record.value?.id);
const previewUrl = ref(null);

const form = useForm({
    titulo: record.value?.titulo ?? '',
    imagen: null,
...
const submit = () => {
    ...
    if (isEdit.value) {
        form.transform((data) => ({
            ...
            _method: 'put',
        })).post(route('admin.banners.update', record.value.id), options);
    } else {
        form.transform((data) => ({
            ...data,
            activo: data.activo ? 1 : 0,
...
<template>
    <Head :title="isEdit ? 'Editar banner — GreenPoint' : 'Nuevo banner — GreenPoint'" />

    <AuthenticatedLayout>
        <template #header>
            <h3 class="mb-0">{{ isEdit ? 'Editar banner' : 'Nuevo banner' }}</h3>
        </template>
        <template #breadcrumb>
            <li class="breadcrumb-item"><Link :href="route('dashboard')">Home</Link></li>
            <li class="breadcrumb-item">
                <Link :href="route('admin.banners.index')">Banners</Link>
            </li>
