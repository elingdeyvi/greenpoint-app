...
const record = computed(() => props.cliente ?? props.item);
...
const submit = () => {
    ...
    if (isEdit.value) {
        form.transform((data) => ({
            ...
            _method: 'put',
        })).post(route('admin.clientes.update', record.value.id), options);
    } else {
        form.transform((data) => ({
            ...data,
            activo: data.activo ? 1 : 0,
        })).post(route('admin.clientes.store'), options);
    }
};
</script>

<template>
    <Head :title="isEdit ? 'Editar cliente — GreenPoint' : 'Nuevo cliente — GreenPoint'" />

    <AuthenticatedLayout>
        <template #header>
            <h3 class="mb-0">{{ isEdit ? 'Editar cliente' : 'Nuevo cliente' }}</h3>
        </template>
        <template #breadcrumb>
            <li class="breadcrumb-item"><Link :href="route('dashboard')">Home</Link></li>
            <li class="breadcrumb-item">
                <Link :href="route('admin.clientes.index')">Clientes</Link>
            </li>
                    ...
                    <form @submit.prevent="submit">
                        <div class="card-body">
                            ...
                            <div class="mb-3">
                                ...
                                <div v-if="previewUrl || record?.logo" class="mt-2">
                                    <img
                                        :src="previewUrl || `/storage/${record.logo}`"
                                        alt="Vista previa"
                                        class="img-thumbnail"
                            ...
                            <div class="mb-3">
                                ...
                                <input
                                    ...
                                    placeholder="https://"
                                />
                                <div v-if="form.errors.enlace" class="invalid-feedback">
                                    {{ form.errors.enlace }}
                                </div>
                            </div>
                            <div class="mb-3">
